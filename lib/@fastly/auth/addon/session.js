...
export default class Session {
  ...
  addHeaders(options) {
    ...
    if (this.token) {
      options.headers[FASTLY_KEY_HEADER] = this.token.accessToken || this.token.access_token
    }
  ...
  async switchAccount(customerId, { retry = true, returnTo, otp, login } = {}) {
    ...
    try {
      ...
      const accountToken = extractPayload(
        await this.ajax(`/account-authorizations/${customerId}`, { type: 'post', ...options })
      )
      ...
      if ([400, 401].includes(errorResponse.status) && this.ssoLoginUrl) {
        ...
        if (errorResponse && errorResponse.responseJSON && errorResponse.responseJSON.error === 'sso_recommended') { // eslint-disable-line
...
function adjustCustomerIdQueryParam(session) {
  ...
  const customerId = href.searchParams.get('customerId') // eslint-disable-line ember/use-ember-get-and-set
  const canUseCustomerId = ['internal', 'salesadmin', 'admin'].includes(user.role)

  if (canUseCustomerId && !customerId && currentCustomerId) {
    href.searchParams.set('customerId', currentCustomerId) // eslint-disable-line ember/use-ember-get-and-set
