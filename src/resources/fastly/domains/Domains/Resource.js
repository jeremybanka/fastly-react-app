...
function Domains(props: Props): React.Node {
  ...
  const qs = stringify({ sum_by: 'domain', metric: metrics.join(','), start, end, downsample, region });

  const url = `/metrics/domains/services/${serviceId}?${qs}`;
  const options = {
    headers: {
