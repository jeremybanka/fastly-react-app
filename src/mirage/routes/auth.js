import getDataFromRequest from "./get-data-from-request"
...
export default function AuthRoutes({ server, origin = "" }) {
  ...
  server.get(
    `${origin}/customers`,
    function (schema: Schema, request: Request) {
      const nameFilter = (
        request.queryParams["filter[name][match]"] || ""
      ).toLowerCase()
      return schema.customers.where(
        (c: any) => c.name && c.name.toLowerCase().includes(nameFilter)
      )
    }
  )
  server.get(`${origin}/customers/:id`, "customer")

  server.get(`${origin}/services`, function (schema: Schema, request: Request) {
    const nameFilter = (
      request.queryParams["filter[name][match]"] || ""
    ).toLowerCase()
    ...
    return schema.services.where((s: any) =>
