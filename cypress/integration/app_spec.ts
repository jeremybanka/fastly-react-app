...
function signIn(
  server: MirageServer,
  { user: userOverrides }: { user?: FixAnyPlease } = {},
  features: Features[] = [],
  ...
  const userOptions = Object.assign({}, { customer, role: `superuser` }, userOverrides)
...
describe(`My First Test`, () => {
  ...
  it(`shows the user_id and customer_id for fastly customer`, () => {
    cy.mirage((server) => signIn(server, { user: { login: `testadmin@fastly.com` } }))
    cy.visit(`http://localhost:3000/tls-configurations`)
  ...
  it(`shows the feature-check false when not enabled`, () => {
    ...
    cy.visit(`http://localhost:3000/tls-configurations`)
    cy.get(`[data-testid=feature-check]`).contains(`Feature-check: exemptFromTlsBilling: false`)
  })

  it(`shows the feature-check true when enabled`, () => {
    cy.mirage((server) => signIn(server, {}, [Features.exemptFromTlsBilling]))
    cy.visit(`http://localhost:3000/tls-configurations`)
    cy.get(`[data-testid=feature-check]`).contains(`Feature-check: exemptFromTlsBilling: true`)
  })
